...
public class WorldManager : MonoBehaviour
{
    ...
    private void AddNeighbor(Point p, Vector3Int neighbor)
    {
        if (neighbor.x > -1 && neighbor.x < GridWidth &&
           neighbor.y > -1 && neighbor.y < GridHeight &&
           neighbor.z > -1 && neighbor.z < GridLength)
        {
            p.Neighbors.Add(neighbor);
        }
    }
        ...
        for (int i = 0; i < GridWidth; i++)
        {
            ...
            for (int j = 0; j < GridHeight; j++)
            {
                ...
                for (int k = 0; k < GridLength; k++)
                {
                    ...
                    //if there is an object in this grid cube, mark it as invalid
                    if (Physics.CheckBox(Grid[i][j][k].WorldPosition, Vector3.one * PointDistance / 2f, Quaternion.identity))
                    {
                        Grid[i][j][k].Invalid = true;
                    }
                    //adds points one space adjacent to the current point to its list of neighbors
                    for (int p = -1; p <= 1; p++)
                    {
                        for (int q = -1; q <= 1; q++)
                        {
                            for (int g = -1; g <= 1; g++)
                            {
                                ...
                                AddNeighbor(Grid[i][j][k], new Vector3Int(i + p, j + q, k + g));
                            }
                        }
                    }
                }
